...
import type { Routine, RoutineExercise } from '@/types/gym';
...
const Routines: React.FC = () => {
  ...
  const handleSave = async () => {
    if (editing) {
      ...
      // Update exercises: delete all and re-insert
  ...
  const handleEdit = (routine: Routine) => {
    setEditing(routine);
    setForm({
      name: routine.name, goal: routine.goal, level: routine.level,
      days_per_week: routine.days_per_week.toString(), description: routine.description || '',
    });
  ...
  const goalLabels: Record<string, string> = {
    muscle_gain: 'Masa Muscular', fat_loss: 'Pérdida de Grasa',
    maintenance: 'Mantenimiento', beginner: 'Principiante',
  };
            ...
            <div className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input type="number" min="1" max="7" value={form.days_per_week} onChange={e => setForm({ ...form, days_per_week: e.target.value })} />
                </div>
                <div>
                  ...
                  {exercises.map((ex, i) => (
                    <div key={i} className="grid grid-cols-12 gap-2 items-end p-3 rounded-lg bg-muted/50">
                      <div className="col-span-12 sm:col-span-4">
                        ...
                        <Input value={ex.exercise_name} onChange={e => updateExercise(i, 'exercise_name', e.target.value)} placeholder="Nombre" />
                      </div>
                      ...
                      <div className="col-span-3 sm:col-span-2">
                        ...
                        <Input type="number" value={ex.rest_seconds} onChange={e => updateExercise(i, 'rest_seconds', e.target.value)} />
                      </div>
                      <div className="col-span-3 sm:col-span-2 flex justify-end">
                        {exercises.length > 1 && (
                          <Button type="button" variant="ghost" size="icon" onClick={() => removeExercise(i)} className="hover:text-destructive">
              ...
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    ...
                    {isAdmin && (
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(routine.id)} className="hover:text-destructive">
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    )}
                  </div>
