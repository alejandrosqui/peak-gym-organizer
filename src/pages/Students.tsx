...
const Students: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    setDialogOpen(false); setEditing(null); setForm(emptyStudent); fetchStudents();
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { setEditing(null); setForm(emptyStudent); } }}>
          ...
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <Button onClick={handleSave} className="w-full mt-4" disabled={!form.full_name}>{editing ? 'Guardar Cambios' : 'Crear Alumno'}</Button>
          </DialogContent>
        </Dialog>
      </div>
